...
Group the storm data set by STATE and EVENT type(EVTYPE1), summarize the values for totals of  FATALITIES and INJURIES and then sort by highest FATALITIES first followed by highest INJURIES.
```{r groupevent, echo = TRUE,results='asis'}
StormSubsetHealth2 <- group_by(StormSubsetHealth, EVTYPE1, STATE)
StormSubsetHealth3 <- summarise(StormSubsetHealth2, FATALITIES=sum(FATALITIES), 
INJURIES=sum(INJURIES))
StormSubsetHealth3 <- arrange(StormSubsetHealth3, desc(FATALITIES), desc(INJURIES))
colnames(StormSubsetHealth1) <- c("EVENT", "STATE" "FATALITIES", "INJURIES")
```
