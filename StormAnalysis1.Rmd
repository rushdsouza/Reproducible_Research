...
kable(head(StormSubsetHealth4,10), row.names=FALSE, align="c")
```
...
kable(head(StormSubsetHealth5,10), row.names=FALSE, align="c")
```
...
StormSubsetCrop <- StormSubsetCrop[(StormSubsetCrop$CROPDMG != 0),]
```
...
kable(cexp, row.names=FALSE, align="c")
```
...
kable(head(StormSubsetCrop2[, c("EVENT","CROP DAMAGE COST")],10), row.names=FALSE, align="c")
```
...
StormSubsetProp <- StormSubsetProp[(StormSubsetProp$PROPDMG != 0),]
```
...
kable(pexp, row.names=FALSE, align="c")
```
...
kable(head(StormSubsetProp2[, c("EVENT","PROPERTY DAMAGE COST")],10), row.names=FALSE, align="c")
```

Group the Property damage data set by EVENT type(EVTYPE1) and STATE(STATE), summarize the values for Property Damage cost(PROPDMGCOST) and then sort by highest PROPDMGCOST first.
```{r groupeventstateprop, echo = TRUE,results='hide'}
StormSubsetPropState1 <- with(StormSubsetProp, aggregate(PROPDMGCOST,by=list(YEAR=BGN_YEAR,EVENT=EVTYPE1, STATE=STATE), sum))
colnames(StormSubsetPropState1) <- c("YEAR","EVENT","STATE", "PROPDMGCOST")

StormSubsetPropState2 <- StormSubsetPropState1[order(-StormSubsetPropState1$YEAR,-StormSubsetPropState1$PROPDMGCOST),]
source('C:/russel/CurrencyFormat.R')
StormSubsetPropState2 <- StormSubsetPropState2[(StormSubsetPropState2$PROPDMG != 0),]

StormSubsetPropState2$PROPERTY_DAMAGE_COST <- ifelse((StormSubsetPropState2$PROPDMGCOST > 1000000 & StormSubsetPropState2$PROPDMGCOST < 1000000000) , 
paste(signif(StormSubsetPropState2$PROPDMGCOST/1000000, digits=5),"M", sep=" "), StormSubsetPropState2$PROPDMGCOST) 

StormSubsetPropState2$PROPERTY_DAMAGE_COST <- ifelse(StormSubsetPropState2$PROPDMGCOST > 1000000000, 
paste(signif(StormSubsetPropState2$PROPDMGCOST/1000000000, digits=5),"B", sep=" "), StormSubsetPropState2$PROPERTY_DAMAGE_COST) 

StormSubsetPropState2$PROPERTY_DAMAGE_COST <- ifelse(StormSubsetPropState2$PROPDMGCOST < 1000000, 
paste(signif(StormSubsetPropState2$PROPDMGCOST/1000, digits=5),"K", sep=" "), StormSubsetPropState2$PROPERTY_DAMAGE_COST) 

# StormSubsetPropState2$PROPERTY_DAMAGE_COST <- CurrencyFormat(StormSubsetPropState2$PROPDMGCOST,F)

colnames(StormSubsetPropState2) <- c("YEAR","EVENT", "STATE", "PROPDMGCOST", "PROPERTY DAMAGE COST")
```

The top 10 Storm Data Events with greatest economic consequences being Property damage are as shown below.
```{r proptop10eventstate, echo = TRUE,results='asis'}
# kable(head(StormSubsetPropState2[, c("STATE", "EVENT", "PROPERTY DAMAGE COST")],10))
```

```{r proptop10eventstate1, echo = TRUE,results='asis'}
year <- unique(StormSubsetPropState2$YEAR)
par(mfrow=c(1,3))
for (i in 1:length(year)) {
print(kable(head(StormSubsetPropState2[ StormSubsetPropState2$YEAR == year[i], c(1:3,5)],10), row.names=FALSE, align="c"))
}
```
